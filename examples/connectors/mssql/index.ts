...
async function main() {
  ...
  // Create table
  await db.sql`IF OBJECT_ID('users', 'U') IS NOT NULL DROP TABLE users`;
  await db.sql`CREATE TABLE users (id INT IDENTITY(1,1) PRIMARY KEY, name NVARCHAR(255), email NVARCHAR(255))`;
